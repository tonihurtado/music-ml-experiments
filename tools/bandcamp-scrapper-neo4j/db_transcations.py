...
from models import Genre, Fan, Album, Artist
...
def getGenre(name):
    with db.read_transaction:
        genre = Genre.nodes.first_or_none(name=name)
    return genre
...
def getArtist(name):
    with db.read_transaction:
        artist = Artist.nodes.get_or_none(name=name)
    return artist
...
def createArtist(name, temp_id):
    with db.read_transaction:
        artist = Artist.nodes.get_or_none(name=name)
    if artist == None:
        with db.write_transaction:
            artist = Artist(name=name, temp_id=temp_id).save()
    return artist
...
def addArtisttoAlbum(artist: Artist, album: Album):
    if artist not in album.getArtist():
        album.artist.connect(artist)
        artist.save()
        album.save()
